...
macro_rules! fuzz_test {
    ($seed: expr, $fuzzer:path, $parser:path, $anarchy_level:path) => {
        for _ in 0..({
            ...
            $fuzzer(&mut rng, &mut qb);
            if let Err(err) = $parser(qb.as_str()) {
                let err = err.to_spanned_error(&qb);
                eprintln!(
                    "{}",
...
macro_rules! fuzz_test_compare {
    ($seed: expr, $fuzzer:path, $parser:expr, $parser2:expr, $anarchy_level:path) => {
        for _ in 0..({
            ...
            let mut qb = String::new();
            $fuzzer(&mut rng, &mut qb);
            let qb_trimmed = qb.trim_matches(|c| c == ' ' || c == '\t' || c == '\n' || c == '\r');
            let res = $parser(qb_trimmed).map_err(|err| err.to_spanned_error(&qb));
            let res2 = $parser2(qb_trimmed).map_err(|err| err.to_spanned_error(&qb));
            if res != res2 {
                if let Err(err) = &res {
                    let err = err.to_spanned_error(&qb);
                    eprintln!(
                ...
                if let Err(err2) = &res2 {
                    let err2 = err2.to_spanned_error(&qb);
                    eprintln!(
                        "{}",
                        DisplayList::from(snippet_for_input_and_part(
