...
import * as wasm from "datapet-codegen-wasm";
import testDtpt from "./test.dtpt";

const Page = () => {
    const [text, setText] = useState(testDtpt);
    const [dtpt, setDtpt] = useState(testDtpt);
    ...
    useEffect(() => {
        if (trackPosition) {
            if (inputRef.current) {
                setDtpt(inputRef.current.value.substring(0, inputRef.current.selectionStart));
            }
        } else {
            setDtpt(text);
        }
    ...
    return (<>
        ...
        <div style={{width: "100%", display: "flex", height: "70vh", gap: 20}}>
            <textarea
                ...
                onClick={e => {
                    if (trackPosition) {
                        setDtpt(e.target.value.substring(0, e.target.selectionStart));
                    } else {
                        setDtpt(e.target.value);
                    }
                }}
                onKeyUp={e => {
                    if (trackPosition) {
